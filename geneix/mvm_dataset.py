...
def mvm_tensor(zeros, shift_add_bit_stream, shift_add_bit_slice, output_reg, flatten_input, flatten_input_sign, bias_addr, xbars, bit_slice, bit_stream, weight_bits, weight_bit_frac, input_bits, input_bit_frac, adc_bit, acm_bit, acm_bit_frac, xbar_row_size, xbar_col_size, G_real, dataset):   # version 2
...
#                if xbars_row1 >1:
#                    xbars_row1 = xbars_row1//2
#                if xbars_col1 >2:
#                    xbars_col1 = xbars_col1//2
#                row_list = 
                #64x64, 32x32, 16x16
                xbars_row1 = 2 if xbars_row1 > 2 else xbars_row
                xbars_col1 = 2 if xbars_col1 > 2 else xbars_col
                #128x128
#                xbars_row1 = 3 if xbars_row1 > 3 else xbars_row
#                xbars_col1 = 3 if xbars_col1 > 3 else xbars_col                
...
def mvm_tensor_ind(zeros, shift_add_bit_stream, shift_add_bit_slice, output_reg, output_analog, Goffmat, G_real_flatten, G_real, model, loop, flatten_input,
                   ...
                   acm_bit_frac, xbar_row_size, xbar_col_size):  #### These should be 'almost' completely changed. 
...
#    Goffmat = Goff*torch.ones(XBAR_ROW_SIZE,XBAR_COL_SIZE).to(device)
    ...
    inmin_V = 0
    #100k
    if xbar_row_size == 128:
        inmax_test = 1.2905
        inmin_test = 0.8473          
    elif xbar_row_size == 64:
        inmax_test = 1.0944
        inmin_test = 0.8609
#    #    #100k
#        inmax_test =    1.2
#        inmin_test =    0.85        
    elif xbar_row_size == 32:
        inmax_test = 1.0156
        inmin_test = 0.8742
    elif xbar_row_size == 16:
        inmax_test = 0.9935
        inmin_test = 0.8824 
#        inmax_test =    1.22
#        inmin_test =    0.81
...
#                pdb.set_trace()
#                output_niratio_unscale= output_niratio_unscale/output_niratio_unscale
