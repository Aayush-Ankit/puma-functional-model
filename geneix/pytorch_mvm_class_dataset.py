...
print(parentDir)
#newPath = os.path.join(parentDir, 'nas_crossbar/search/')
#print(newPath)
...
import math
import numpy as np
from mvm_function_dataset import *
import pdb
import time
...
class Conv2d_mvm_function(Function):
    ...
    # |            MVM           |   
    # +--------------------------+
    def forward(ctx, input, weight, xbmodel, xbar_row_size, xbar_col_size, bias=None, stride=1, padding=0, dilation=1, groups=1, bit_slice=2, bit_stream=1, 
                weight_bits=16, weight_bit_frac=-1, input_bits=16, input_bit_frac=-1, adc_bit=-1, acm_bits=16, acm_bit_frac=-1, ind=False, loop = True, tile_row=4,
                tile_col=4, dataset=False):
...
#        print('stride:',stride)
#        print('xbarrows:',xbar_row)
#        print('xbarcol:',xbar_col)
...
#                pdb.set_trace()
                if ind == True:
                    ...
                    xbars_out = mvm_tensor_ind(zero_mvmtensor, shift_add_bit_stream, shift_add_bit_slice, output_reg, output_analog, Goffmat, G_real_flatten0, G_real0, 
                                               ...
                                               weight_bit_frac, input_bits, input_bit_frac, adc_bit, acm_bits, acm_bit_frac, xbar_row_size, xbar_col_size) - \
...
#                    dataset_=False
                    xbars_out = mvm_tensor(zero_mvmtensor, shift_add_bit_stream, shift_add_bit_slice, output_reg, flatten_binary_input_xbar, flatten_input_sign_xbar, 
                                           ...
                                           acm_bit_frac, xbar_row_size, xbar_col_size, G_real0, dataset_) - \
                                mvm_tensor(zero_mvmtensor, shift_add_bit_stream, shift_add_bit_slice, output_reg, flatten_binary_input_xbar, flatten_input_sign_xbar,
                                           ...
                                           acm_bit_frac, xbar_row_size, xbar_col_size, G_real1, False)
...
#            grad_bias = grad_output.sum((0,2,3)).squeeze(0)
        ...
        return grad_input, grad_weight, grad_bias, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None 
...
class _ConvNd_mvm(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, xbmodel, pretrained_model_path, xbar_row_size, xbar_col_size, kernel_size, stride, padding, dilation, transposed,
                 output_padding, groups, bias, padding_mode, bit_slice, bit_stream, weight_bits, weight_bit_frac, input_bits, input_bit_frac, adc_bit, acm_bits,
                 acm_bit_frac, ind, loop, tile_row, tile_col, dataset=False, check_grad=False):
        ...
        self.xbar_row_size = xbar_row_size
        self.xbar_col_size = xbar_col_size
        self.tile_row = tile_row
        self.tile_col = tile_col
        self.xb_size = str(xbar_row_size)+'x'+str(xbar_col_size) #string to show the crossbar size
...
class Conv2d_mvm(_ConvNd_mvm):
    def __init__(self, in_channels, out_channels, xbmodel, pretrained_model_path, xbar_row_size, xbar_col_size, kernel_size, stride=1, padding=0, dilation=1, groups=1,
                 bias=True, padding_mode='zeros', check_grad=False, bit_slice=2, bit_stream=1, weight_bits=16, weight_bit_frac=-1, input_bits=16, input_bit_frac=-1,
                 adc_bit=-1, acm_bits=16, acm_bit_frac=-1, ind=False, loop = True, tile_row=4, tile_col=4, dataset=False):
        ...
        super(Conv2d_mvm, self).__init__(
            in_channels, out_channels, xbmodel, pretrained_model_path, xbar_row_size, xbar_col_size, kernel_size, stride, padding, dilation,
            ...
            acm_bit_frac, ind, loop, tile_row, tile_col, dataset)
    #@weak_script_method
    def forward(self, input):
            return Conv2d_mvm_function.apply(input, self.weight, self.xbmodel, self.xbar_row_size, self.xbar_col_size, self.bias, self.stride, self.padding,
                                             self.dilation, self.groups, self.bit_slice, self.bit_stream, self.weight_bits, self.weight_bit_frac, self.input_bits, 
                                             self.input_bit_frac, self.adc_bit, self.acm_bits, self.acm_bit_frac, self.ind, self.loop, self.tile_row, self.tile_col, self.dataset)
...
class Linear_mvm_function(Function):
    ...
    def forward(ctx, input, weight, xbmodel, xbar_row_size, xbar_col_size, bias=None, bit_slice=2, bit_stream=1, weight_bits=16, weight_bit_frac=-1, input_bits=16,
                input_bit_frac=-1, adc_bit=-1, acm_bits=16, acm_bit_frac=-1, ind=False, loop = True, dataset=False):
...
#        binary_input[:,:input.shape[1]] = float_to_16bits_tensor(input, input_bit_frac, bit_stream, input_bits, device)   # batch x n x 16
        ...
        if ind == True:
            xbars_out = mvm_tensor_ind(zero_mvmtensor, shift_add_bit_stream, shift_add_bit_slice, output_reg, output_analog, Goffmat, G_real_flatten0, G_real0, 
                                       ...
                                       input_bits, input_bit_frac, adc_bit, acm_bits, acm_bit_frac, xbar_row_size, xbar_col_size) - \
                        mvm_tensor_ind(zero_mvmtensor, shift_add_bit_stream, shift_add_bit_slice, output_reg, output_analog, Goffmat, G_real_flatten1, G_real1, 
            ...
            xbars_out = mvm_tensor(zero_mvmtensor, shift_add_bit_stream, shift_add_bit_slice, output_reg, binary_input, input_sign_xbar, bias_addr, xbars[0],
                                   bit_slice, bit_stream, weight_bits, weight_bit_frac, input_bits, input_bit_frac, adc_bit, acm_bits, acm_bit_frac, xbar_row_size, xbar_col_size, G_real0, dataset) - \
                        mvm_tensor(zero_mvmtensor, shift_add_bit_stream, shift_add_bit_slice, output_reg, binary_input, input_sign_xbar, bias_addr, xbars[1], 
                                   bit_slice, bit_stream, weight_bits, weight_bit_frac, input_bits, input_bit_frac, adc_bit, acm_bits, acm_bit_frac, xbar_row_size, xbar_col_size, G_real1, False)
...
#            grad_bias = grad_output.sum(0).squeeze(0)
        ...
        return grad_input, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None 

class Linear_mvm(nn.Module):
    def __init__(self, input_features, output_features, xbmodel, pretrained_model_path, xbar_row_size, xbar_col_size, bias=True, bit_slice = 2, bit_stream = 1, 
                 weight_bits=16, weight_bit_frac=-1, input_bits=16, input_bit_frac=-1, adc_bit=-1, acm_bits=16, acm_bit_frac=-1, ind = False, loop = True, dataset=False):
    ...
    def forward(self, input):
        ...
        return Linear_mvm_function.apply(input, self.weight, self.xbmodel, self.xbar_row_size, self.xbar_col_size, self.bias, self.bit_slice, self.bit_stream, 
                                         self.weight_bits, self.weight_bit_frac, self.input_bits, self.input_bit_frac, self.adc_bit, self.acm_bits, self.acm_bit_frac,
                                         self.ind, self.loop, self.dataset)

    def extra_repr(self):
        # (Optional)Set the extra information about this module. You can test
        # it by printing an object of this class.
        return 'in_features={}, out_features={}, bias={}, xb_size={}'.format(
            self.input_features, self.output_features, self.bias is not None, str(self.xbar_row_size)+'x'+str(self.xbar_col_size)
        )
